...
namespace Json {
  ...
  const std::map<reservedToken, std::string_view> reversedCharacterMap = ([]() {
    ...
    for (const auto& element : specialCharacterMap) { ret.emplace(element.second, element.first); }
    return ret;
  })();
  ...
  Token getString(preprocStream& stream)
  {
    ...
    for (; getCharType(c) != CharType::eof; c = stream.getc()) {
      ...
      switch (c) {
      case '"': return Token(std::move(str));
  ...
  Token tokenize(preprocStream& stream)
  {
    while (true) {
      ...
      case CharType::numeric: stream.push_back(c); return getNumber(stream);
      }
      break;
    }
  }
